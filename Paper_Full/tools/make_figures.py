#!/usr/bin/env python3
"""Generate all Paper_Full vector figures from released/audited result files."""
...
DATA = json.loads((ROOT / "data" / "audited_results.json").read_text())
...
def save(fig: plt.Figure, name: str) -> None:
    for suffix in ("pdf", "png"):
        ...
        metadata = (
            {"Creator": "Paper_Full/tools/make_figures.py", "CreationDate": None, "ModDate": None}
            if suffix == "pdf"
            else {"Software": "Paper_Full/tools/make_figures.py"}
        )
        fig.savefig(
            OUT / f"{name}.{suffix}",
