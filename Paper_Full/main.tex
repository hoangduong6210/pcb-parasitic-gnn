...
The primary comparison uses the per-design ratio between paired solver time and raw-layout end-to-end inference. Its median is $670.2\times$, with 95\% bootstrap interval $[640.8,710.4]\times$. For an interactive batch-one path, this is the relevant boundary. A historical 1.17\,ms figure came from pre-collated batch throughput; the same protocol produced 0.675\,ms on the current node and runtime stack, confirming that the absolute number depends on hardware and software. Combining the rounded 5\,s solver estimate with the historical throughput gives approximately $4300\times$, but that calculation does not include the raw-layout end-to-end boundary.
...
The capacitance FEM uses an effective dielectric description. Layered anisotropic dielectrics, frequency-dependent loss tangent, and full-wave coupling are outside its scope. These omissions matter most when the surrogate is moved from layout ranking to absolute EMI prediction.
...
\section{Conclusion}
Replacing analytical labels with solver outputs reduces held-out median error to a range of 1.75 to 3.31\% across $\Cps$, $\Lp$, $\Ls$, and $\Mut$. An independent Neumann formulation bounds the remaining inductive discrepancy. Graph structure contributes most on relational targets, size transfer, and fine layout decisions. A pairwise objective improves within-family ordering and nearly recovers the optimal lateral-registration choice where the analytical model is blind.

The corrected EGNN supplies strict $E(3)$ behavior on encoded graphs, but its predictive effect remains unresolved against a parameter-matched invariant model. Latency depends on the stated boundary: the raw-layout path is 5.62\,ms/design and gives a paired end-to-end median speedup of $670\times$ over the evaluated field workflow. The result supports solver-grade screening on synthetic geometries. Measured boards, richer layout primitives, layered dielectric references, and joint core-winding models remain outside the present validation.
...
The primary $670.2\times$ result is therefore the median of paired per-design solver/end-to-end ratios. Its 95\% interval $[640.8,710.4]\times$ resamples designs. The pre-collated throughput is reported separately and applies only when graph preparation is outside the timed loop or amortized over a candidate bank. Hardware, thread count, warm-up, batch size, repeat count, and timer boundary remain necessary context when comparing latency with another system.
