...
import corpus_v4_accuracy_contract as contract  # noqa: E402
import finalize_corpus_v4_accuracy as finalizer  # noqa: E402
import plan_corpus_v4_accuracy as planner  # noqa: E402
import plan_corpus_v4_accuracy_resume as resume  # noqa: E402
import run_corpus_v4_accuracy_task as runner  # noqa: E402
...
def test_finalizer_reconstructs_three_scopes_without_r4_substitution() -> None:
    ...
    for layout_id in range(39):
        reference = [float(layout_id + 1), float(layout_id + 2), float(layout_id + 3), float(layout_id + 4)]
    ...
    metrics = finalizer.recompute_task_metrics(predictions, task, evaluation)
    ...
    with pytest.raises(ValueError, match="identity mismatch"):
        finalizer.recompute_task_metrics(tampered, task, evaluation)


def test_crossed_interval_is_deterministic_and_labeled_descriptive() -> None:
    protocol = json.loads((ROOT / "protocols/corpus_v4_accuracy_v1.json").read_text())
...
def _protocol() -> dict[str, Any]:
    return json.loads((ROOT / "protocols/corpus_v4_accuracy_v1.json").read_text())
...
def _training_environment(
    ...
    *,
    ...
    values = {
        ...
        "SLURM_JOB_ID": job_id or f"8100000_{task_id}",
...
def _training_scheduler_fields(
    *,
    ...
    return {
        ...
        "JobId": job_id or f"8100000_{task_id}",
        "JobState": "RUNNING",
        "MinMemoryNode": "48G",
...
def _write_checkpoint_only_attempt(
    ...
    validation_ids = [101, 102, 103, 104, 105]
    test_ids = [201, 202, 203, 204, 205]
    smoke_rows = [
        {
            "geometry_sha256": f"{layout_id:064x}",
            "layout": {"fixture": layout_id},
            "layout_id": layout_id,
        }
    ...
    curve = [
        {
            "epoch": epoch,
            "learning_rate": 1e-3,
            "schema": "pcb-gnn.corpus-v4-accuracy-learning-curve.v1",
    ...
    source_hashes = {"code/example.py": "f" * 64}
    result: dict[str, Any] = {
        "bindings": {
        ...
        "provenance": {
            ...
            "software": contract.runtime_identity(),
...
@pytest.mark.parametrize(
    "field",
    (
        "test_or_r4_used_for_normalization_optimization_or_acceptance",
...
def _accepted_set_fixture(
    ...
    candidate = {
        ...
        "schema": "pcb-gnn.corpus-v4-accuracy-candidate-index.v1",
    }
...
def test_batch_resources_and_no_login_training_contract() -> None:
    training = (ROOT / "code/jobs/submit_corpus_v4_accuracy.sh").read_text()
    final = (ROOT / "code/jobs/submit_finalize_corpus_v4_accuracy.sh").read_text()
