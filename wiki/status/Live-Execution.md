...
Last scheduler-backed observation: 2026-08-29 17:11 UTC.
...
| FEM-v2 accuracy v3 | `FROZEN; R2 PREFLIGHT PENDING` | Five split-scoped training artifacts and one held-out commitment; jobs `7086917` and `7086936` failed closed before data loading | Submit the singleton from clean source commit `c0ffca0d0637e8fbba81c126c3f56f8316003a9a` and lock r2 | The first attempt used an unsupported Bubblewrap option. R1 then stopped at scheduler self-authentication. R2 adds read-only configless-SLURM client state; training and downstream gates remain closed. |
...
protocol-v3 r2 singleton sandbox preflight from source commit
`c0ffca0d0637e8fbba81c126c3f56f8316003a9a`. Only a passing preflight permits
