...
| `C-BASE-FEMV2-001` | `RUNNING; PROTOCOL FROZEN` | A fixed baseline extension is running on the already evaluated FEM-v2 benchmark after its filesystem preflight passed. No relative model-accuracy outcome is admitted. | Complete checkpoint acceptance, held-out finalizer, archive replay and review; execution indexed by `E-C4-BASE-FEMV2-PLAN-01`. |
