...
| Tracked replay | `7326296`, COMPLETED/0:0, zero restarts, 229 s on `a0126`; clean source `3157021d35b06a9e9bc20777dcb50d047b85d606` |
| Tracked receipt | [Receipt](../../results/corpus_v4/strict_e3_fem_v2/recovery/v1/verify/job_7326296/result.json), SHA-256 `3d578a6411a06ee28bc9a15e962619439e307be0116f80cd2645d01445a282fd` |
...
| Archive SHA-256 | `77b19ceacf78db1ea9768ec6c9993dd6517c68ca10e1c96bb9dfc1ea2ea185f0` |
...
does not by itself establish baseline accuracy or a benefit from graph
connectivity. The subsequent accepted set, analysis and two replay jobs
validate the numerical comparison. Its exact wording and limitations are
recorded in the [baseline result page](../results/Corpus-V4-FEM-v2-Baselines.md).
...
| Rejected execution lock r1 | [`corpus_v4_accuracy_execution_lock_v3r1.json`](../../protocols/corpus_v4_accuracy_execution_lock_v3r1.json), SHA-256 `e8916801d3479f06b6eb71477796c4ae1b15408bf90aa7e516ad8ac7c02adbf0` |
| Active execution lock r2 | [`corpus_v4_accuracy_execution_lock_v3r2.json`](../../protocols/corpus_v4_accuracy_execution_lock_v3r2.json), SHA-256 `8f70369457382ab1d4066e194b2f4664813ece98deb514628ead27fb365c5e8c` |
...
| Isolation | Hash-pinned Bubblewrap sandbox; exactly one training artifact is mounted; repository data, held-out evaluation, other split artifacts, `.git`, and host user roots are absent from the local mounted filesystem. The host network namespace is shared for SLURM self-authentication. |
...
## E-C4-LAT-FEMV2-PLAN-01: Frozen paired-latency preflight plan
...
| Builder print failure | The initial relative `--out` wrote the immutable admission before its final path-print raised `ValueError`. Independent replay succeeded on the exact saved file without overwriting it; the documented command now passes an absolute output path. |
| Diagnostic logs | [`preflight/logs/`](../../results/corpus_v4/latency_fem_v2/preflight/logs) contains six stdout/stderr captures; PyTorch's `index_reduce` beta warning is nonterminal. |
| Full-array job | SLURM `7260429` (`0-305%8`), submitted after admission against its SHA; all 306 tasks `COMPLETED/0:0`, no restarts |
| Full accepted set | [`accepted_artifact_set.json`](../../results/corpus_v4/latency_fem_v2/resume/round_00/accepted_artifact_set.json), SHA-256 `90ba1f8d44a5db921a77fad50aec6b99171ef31b4e129fde59202a800ee6d221`; 306 accepted, zero pending |
| Failed analysis attempt | [`job_7271384`](../../results/corpus_v4/latency_fem_v2/diagnostics/job_7271384/README.md): `FAILED/1:0`, 18 s, no restarts. Analysis files were written before a relative-path print error; all remain diagnostic |
...
| Predesignated preflight | Task IDs 0, 152, and 305, submitted with concurrency one from the clean pinned source; all three have terminal accounting and hash-backed numerical/mesh replay |
