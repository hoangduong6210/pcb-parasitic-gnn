#!/usr/bin/env python3
"""Authenticate unchanged latency measurements and a separately locked finalizer.

The timing execution remains bound to the original V2 source root.  Only the
analysis job uses the additional V3 lock; this does not create a new benchmark.
"""
...
from corpus_v4_latency_finalizer_contract_v3 import (  # noqa: E402
    validate_finalizer_execution_lock,
)
from scientific_artifact import atomic_write_json, sha256_file  # noqa: E402


ARCHIVE_SCHEMA = f"{SCHEMA_PREFIX}-archive.v2"
CANDIDATE_SCHEMA = f"{SCHEMA_PREFIX}-candidate-index.v1"


def validate_summary_source_roots(
    summary: dict[str, Any],
    *,
    expected_bindings: dict[str, Any],
    finalizer_lock: dict[str, Any],
    expected_finalizer_source_git_head: str,
) -> None:
    """Reject summaries that confuse measurement and analysis provenance."""
    provenance = summary.get("provenance")
    if (
        summary.get("bindings") != expected_bindings
        or not isinstance(provenance, dict)
        or provenance.get("source_git_head") != expected_finalizer_source_git_head
        or provenance.get("source_sha256") != finalizer_lock["source_sha256"]
    ):
        raise ValueError("summary does not authenticate distinct task/finalizer source roots")
...
def validate_analysis_inventory(analysis_root: Path, expected_names: set[str]) -> None:
    """Require only ordinary files, without hidden links or nested payloads."""
    if analysis_root.is_symlink() or not analysis_root.is_dir():
        raise ValueError("analysis root is not an ordinary directory")
...
def verify(args: argparse.Namespace) -> dict[str, Any]:
    ...
    finalizer_lock, finalizer_lock_sha = validate_finalizer_execution_lock(
        ...
        task_lock=lock,
        task_lock_sha256=lock_sha,
        accepted_set_sha256=args.expected_accepted_set_sha256,
        expected_task_source_git_head=args.expected_source_git_head,
    )
    bindings = {
    ...
    validate_analysis_inventory(
        analysis_root, expected_names | {"ANALYSIS_MANIFEST.json"}
    )
    ...
    finalizer_job_id = str(summary.get("provenance", {}).get("scheduler", {}).get("job_id", ""))
    canonical_analysis_root = (
        ROOT / "results/corpus_v4/latency_fem_v2/final" / f"job_{finalizer_job_id}"
    )
    if (
        not finalizer_job_id.isdigit()
        or analysis_root.absolute() != canonical_analysis_root
        or analysis_root.resolve() != canonical_analysis_root
    ):
        raise ValueError("analysis root is not the canonical finalizer job directory")
    expected_summary_bindings = {
        **bindings,
        "accepted_set_sha256": args.expected_accepted_set_sha256,
        "expected_source_git_head": args.expected_source_git_head,
        "finalizer_execution_lock_sha256": finalizer_lock_sha,
        "expected_finalizer_source_git_head": args.expected_finalizer_source_git_head,
    ...
    validate_summary_source_roots(
        summary,
        expected_bindings=expected_summary_bindings,
    ...
    tracked.extend(
        resolve_repo_path(name, "finalizer source")
