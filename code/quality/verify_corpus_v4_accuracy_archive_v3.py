...
from finalize_corpus_v4_accuracy_v3 import (  # noqa: E402
    REFERENCE_GAP_SCHEMA,
    SCIENTIFIC_SCOPE,
    TASK_METRICS_SCHEMA,
...
from verify_corpus_v4_fem_v2_production_archive import (  # noqa: E402
    verify_archive as verify_fem_v2_archive,
)
...
def _validate_candidate_disposition_reason_codes(payload: dict[str, Any]) -> None:
    """Reject free-form exception text in the persisted candidate audit table."""
    for index, disposition in enumerate(payload["candidate_dispositions"]):
        if (
            disposition["outcome"] == "rejected"
            and disposition["reason_code"] not in CANDIDATE_REJECTION_REASON_CODES
        ):
            raise ValueError(
                f"candidate disposition {index} uses an unrecognized reason code"
            )


def _validate_execution_git_closure(
    expected_source_git_head: str, source_sha256: dict[str, str]
) -> None:
    if not isinstance(expected_source_git_head, str) or len(expected_source_git_head) != 40:
        raise ValueError("execution Git head is malformed")
    ...
    if commit.returncode != 0 or ancestor.returncode != 0:
        raise ValueError("execution Git head is missing or not an ancestor")
    for relative, expected in sorted(source_sha256.items()):
        ...
        if blob.returncode != 0 or hashlib.sha256(blob.stdout).hexdigest() != expected:
            raise ValueError(f"execution Git blob differs from source lock: {relative}")


def _validate_upstream_archive(protocol: dict[str, Any]) -> None:
    archive_path = resolve_repo_path(
        protocol["inputs"]["archive_manifest"]["path"], "upstream FEM-v2 archive"
    )
    replay = verify_fem_v2_archive(archive_path)
    if (
        replay.get("status") != "PASS"
        or replay.get("dataset_admission_sha256")
        != protocol["inputs"]["dataset_admission"]["sha256"]
        or replay.get("result_files_verified") != 1698
        or replay.get("training_may_start") is not False
    ):
        raise ValueError("upstream FEM-v2 archive no longer closes")
...
def _completed_finalizer(scheduler: dict[str, Any]) -> dict[str, str]:
    validate_scheduler_receipt(scheduler, stage="finalizer")
    ...
    return validate_terminal_receipt(scheduler, receipt, stage="finalizer")
...
def verify_archive(args: argparse.Namespace) -> dict[str, Any]:
    ...
    tracked_paths.extend(
        args.plan.parent / name for name in plan["artifact_sha256"]
    )
    tracked_paths.extend(
    ...
    _validate_candidate_disposition_reason_codes(accepted)
    ...
    reference_gap = load_json(analysis_root / "reference_fidelity_gap.json")
    task_metrics = load_json(analysis_root / "task_metrics.json")
    if (
        set(task_index) != {"entries", "schema"}
        or task_index.get("schema")
        != "pcb-gnn.corpus-v4-accuracy-task-index.v3"
        ...
        or matrices.get("schema") != "pcb-gnn.corpus-v4-accuracy-matrices.v3"
        or set(reference_gap) != {
            "complete_selected_registry",
            "interpretation",
            "per_split",
            "schema",
        }
        or reference_gap.get("schema") != REFERENCE_GAP_SCHEMA
        or set(task_metrics) != {"entries", "schema"}
        or task_metrics.get("schema") != TASK_METRICS_SCHEMA
        or not isinstance(task_metrics.get("entries"), list)
        or len(task_metrics["entries"]) != 25
    ):
    ...
    if len(evaluation_rows) != 1500 or [
        row.get("layout_id") for row in evaluation_rows
    ] != list(range(1500)):
    ...
    task_software_records: list[dict[str, Any]] = []
    for task_id, (task_row, accepted_entry, task_entry, checkpoint_entry) in enumerate(
        ...
        if terminal_receipt != accepted_entry["scheduler_completion"]:
            raise ValueError(
                f"accepted scheduler receipt {task_id} is not canonically encoded"
            )
        task_software_records.append(task_result["provenance"]["software"])
    ...
    if task_metrics["entries"] != metric_results:
        raise ValueError("stored task metrics differ from prediction-level reconstruction")
    expected_reference_gap = build_reference_fidelity_gap(task_rows, evaluation)
    if reference_gap != expected_reference_gap:
        raise ValueError("stored reference-fidelity gap differs from frozen references")
    assert_finite_json(task_metrics, "accuracy task metrics")
    assert_finite_json(reference_gap, "reference-fidelity gap")
    ...
    if (
        ...
        or summary.get("schema") != FINAL_SCHEMA
        or summary.get("bindings") != expected_bindings
        or summary.get("designated_downstream_checkpoint")
        != checkpoint_index["entries"][12]
        or summary.get("scientific_scope") != SCIENTIFIC_SCOPE
    ):
        raise ValueError("final summary structure or scientific scope is invalid")
    try:
        created = datetime.fromisoformat(summary["created_utc"])
    except (TypeError, ValueError) as exc:
        raise ValueError("final summary timestamp is invalid") from exc
    if created.tzinfo is None or created.utcoffset() is None:
        raise ValueError("final summary timestamp is not timezone aware")
    if not task_software_records or any(
        record != task_software_records[0] for record in task_software_records[1:]
    ):
        raise ValueError("accepted tasks mix software environments")
    provenance = summary.get("provenance")
    task_software = task_software_records[0]
    expected_finalizer_runtime = {
        **protocol["runtime"],
        "torch_build": task_software["torch_build"],
    }
    if (
    ...
    if args.check:
        ...
        finalizer_receipt = _completed_finalizer(
            summary.get("provenance", {}).get("scheduler", {})
        )
    finalizer_receipt = validate_terminal_receipt(
        summary["provenance"]["scheduler"],
        finalizer_receipt,
        stage="finalizer",
    )
