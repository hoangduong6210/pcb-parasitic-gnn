#!/bin/bash
#SBATCH --job-name=pcb-v4-acc-final
...
: "${PCB_GNN_V4_ACCURACY_ACCEPTED_SET:?Set the repository-relative accepted-set path}"
: "${PCB_GNN_V4_ACCURACY_ACCEPTED_SET_SHA256:?Pin the accepted-set SHA-256}"
: "${PCB_GNN_V4_SOURCE_COMMIT:?Pin the exact clean Git commit}"

"$PCB_GNN_PYTHON" -u code/experiments/proofs/finalize_corpus_v4_accuracy.py \
  ...
  --accepted-set "$PCB_GNN_V4_ACCURACY_ACCEPTED_SET" \
  --expected-accepted-set-sha256 "$PCB_GNN_V4_ACCURACY_ACCEPTED_SET_SHA256" \
  --output-root results/corpus_v4/accuracy/final
