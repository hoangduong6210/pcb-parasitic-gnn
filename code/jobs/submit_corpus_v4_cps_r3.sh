...
export PCB_GNN_MAX_MESH_NODES=3000000
export PCB_GNN_MAX_MESH_TETRAHEDRA=20000000
export PCB_GNN_MAX_RSS_KB=33554432
...
: "${PCB_GNN_CPS_RETRY_TASK_SET:?R3 requires a dense dispatch or retry task set}"
: "${PCB_GNN_CPS_RETRY_TASK_SET_SHA256:?Set the pinned task-set SHA-256}"
...
"$PCB_GNN_PYTHON" -u code/experiments/proofs/run_corpus_v4_cps_multifidelity_task.py \
  ...
  --output-root results/corpus_v4/cps_multifidelity/r3 \
  "${RETRY_ARGS[@]}" \
  --timeout-s 1800
