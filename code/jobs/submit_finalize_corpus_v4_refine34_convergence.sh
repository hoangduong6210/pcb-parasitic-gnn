#!/bin/bash
#SBATCH --job-name=pcb-v4-r34-final
...
: "${PCB_GNN_V4_REFINE34_ARRAY_JOB_ID:?Set completed refine-3/refine-4 array ID}"
"$PCB_GNN_PYTHON" -u \
  code/experiments/proofs/finalize_corpus_v4_refine34_convergence.py \
  --source-array-job-id "$PCB_GNN_V4_REFINE34_ARRAY_JOB_ID" \
