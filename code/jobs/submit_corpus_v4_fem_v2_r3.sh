#!/bin/bash
#SBATCH --job-name=pcb-v4-fem-v2-r3
...
: "${PCB_GNN_V4_FEM_V2_DISPATCH:?Pin one R3 dispatch path}"
: "${PCB_GNN_V4_FEM_V2_DISPATCH_SHA256:?Pin the R3 dispatch SHA-256}"
...
"$PCB_GNN_PYTHON" -u code/experiments/proofs/corpus_v4_fem_v2_production.py run \
  ...
  --manifest results/corpus_v4/cps_reference_v2/production/v1/plan/r3_manifest.jsonl \
  ...
  "${RETRY_ARGS[@]}" \
  --output-root results/corpus_v4/cps_reference_v2/production/v1/attempts/r3
