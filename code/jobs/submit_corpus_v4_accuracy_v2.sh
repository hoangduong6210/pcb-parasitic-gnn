#!/bin/bash
#SBATCH --job-name=pcb-v4-accuracy-v2
...
RETRY_ARGS=()
if [[ -n "${PCB_GNN_V4_ACCURACY_V2_PENDING_SET:-}" || -n "${PCB_GNN_V4_ACCURACY_V2_PENDING_SET_SHA256:-}" ]]; then
  : "${PCB_GNN_V4_ACCURACY_V2_PENDING_SET:?Set the repository-relative pending-set path}"
  : "${PCB_GNN_V4_ACCURACY_V2_PENDING_SET_SHA256:?Pin the pending-set SHA-256}"
  RETRY_ARGS=(
    --pending-set "$PCB_GNN_V4_ACCURACY_V2_PENDING_SET"
    --expected-pending-set-sha256 "$PCB_GNN_V4_ACCURACY_V2_PENDING_SET_SHA256"
  )
fi

"$PCB_GNN_PYTHON" -u code/experiments/proofs/run_corpus_v4_accuracy_task_v2.py \
  ...
  "${RETRY_ARGS[@]}" \
  --output-root results/corpus_v4/accuracy_v2/jobs
