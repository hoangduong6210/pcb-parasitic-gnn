#!/bin/bash
#SBATCH --job-name=pcb-v4-lat-rec
...
: "${PCB_GNN_V4_SOURCE_COMMIT:?Pin the original timing-task Git commit}"
: "${PCB_GNN_V4_LATENCY_FINALIZER_SOURCE_COMMIT:?Pin the exact clean recovery Git commit}"
: "${PCB_GNN_V4_LATENCY_FINALIZER_EXECUTION_LOCK_SHA256:?Pin the recovery lock SHA-256}"

"$PCB_GNN_PYTHON" -u code/experiments/proofs/finalize_corpus_v4_latency_v3.py \
  ...
  --expected-finalizer-execution-lock-sha256 "$PCB_GNN_V4_LATENCY_FINALIZER_EXECUTION_LOCK_SHA256" \
  --expected-finalizer-source-git-head "$PCB_GNN_V4_LATENCY_FINALIZER_SOURCE_COMMIT" \
  --output-root "$PCB_GNN_ROOT/results/corpus_v4/latency_fem_v2/final"
