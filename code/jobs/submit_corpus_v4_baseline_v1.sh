#!/bin/bash
#SBATCH --job-name=pcb-v4-baseline-v1
...
: "${PCB_GNN_BASELINE_ROOT:?Pin the absolute clean execution checkout}"
: "${PCB_GNN_BASELINE_PROTOCOL_SHA256:?Pin the baseline protocol}"
: "${PCB_GNN_BASELINE_LOCK_SHA256:?Pin the baseline execution lock}"
: "${PCB_GNN_BASELINE_SOURCE_COMMIT:?Pin the clean source commit}"
EXECUTION_ROOT="$(cd "$PCB_GNN_BASELINE_ROOT" && pwd -P)"
cd "$EXECUTION_ROOT"
[[ "$(git rev-parse HEAD)" == "$PCB_GNN_BASELINE_SOURCE_COMMIT" ]]
...
[[ "$EXECUTED_BATCH_SHA256" == "$(sha256sum code/jobs/submit_corpus_v4_baseline_v1.sh | cut -d' ' -f1)" ]]
...
TASK_RELATIVE="results/corpus_v4/baseline_fem_v2/jobs/job_${SLURM_ARRAY_JOB_ID}/task_${TASK_PADDED}"
PROBE_ARGS=()
if [[ "${PCB_GNN_BASELINE_PROBE_ONLY:-false}" == true ]]; then
  [[ "$SLURM_ARRAY_TASK_ID" == 0 && "$SLURM_ARRAY_TASK_COUNT" == 1 ]]
  TASK_RELATIVE="results/corpus_v4/baseline_fem_v2/probes/job_${SLURM_ARRAY_JOB_ID}/task_${TASK_PADDED}"
  PROBE_ARGS=(--probe-only)
fi
# A task sees only its own empty output directory, never another split's model.
...
/usr/bin/bwrap \
  ...
  --dir /workspace/results/corpus_v4/baseline_fem_v2 \
  ...
  PCB_GNN_EXECUTED_BATCH_SCRIPT=/workspace/code/jobs/submit_corpus_v4_baseline_v1.sh \
  PCB_GNN_EXECUTED_BATCH_SHA256="$EXECUTED_BATCH_SHA256" \
  PCB_GNN_V4_ACCURACY_V3_HOST_SOURCE_COMMIT="$PCB_GNN_BASELINE_SOURCE_COMMIT" \
  PCB_GNN_V4_ACCURACY_V3_HOST_SOURCE_CLEAN=true \
  PCB_GNN_V4_ACCURACY_V3_SANDBOX_ACTIVE=true \
  /usr/bin/python3 -u code/experiments/proofs/corpus_v4_baseline_v1.py train \
