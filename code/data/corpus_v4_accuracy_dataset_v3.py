...
sys.path.insert(0, str(REPO / "code/quality"))
...
DEFAULT_PROTOCOL = REPO / "protocols/corpus_v4_accuracy_v3.json"
...
R3_FIDELITY_ID = "cps_fem_r3_p16_t1_v2"
R4_FIDELITY_ID = "cps_fem_r4_p16_t1_v2"
...
@dataclass(frozen=True)
class TrainingSplitDataset:
    """One split-scoped preacceptance table with no held-out references."""

    samples: tuple[AccuracySample, ...]
    split_seed: int
    train_layout_ids: tuple[int, ...]
    validation_layout_ids: tuple[int, ...]
    artifact_sha256: str
...
def _validate_protocol(protocol: dict[str, Any]) -> None:
    ...
    if (
        ...
        is not True
        or protocol.get("forbidden_actions", {}).get(
            "heldout_bytes_in_training_namespace"
        )
        is not True
        or protocol.get("forbidden_actions", {}).get(
            "full_dataset_loader_in_training"
        )
        is not True
    ...
    *,
    ...
    for row in rows:
        if set(row) != expected_fields:
            raise CorpusV4AccuracyDatasetError("Cps observation row is not exact")
        ...
        destination[key] = (
            ...
            _require_sha256(
                row.get("artifact_result_sha256"), "Cps result artifact"
            ),
        )
...
def load_training_split_dataset_v3(
    path: Path,
    *,
    expected_sha256: str,
    task_row: Mapping[str, Any],
) -> TrainingSplitDataset:
    """Load exactly one train/validation artifact without touching held-out data."""
    if path.is_symlink() or not path.is_file():
        raise CorpusV4AccuracyDatasetError(
            "training split artifact must be a regular non-symlink file"
        )
    path = path.resolve()
    if not SHA256_RE.fullmatch(expected_sha256) or _sha256_file(path) != expected_sha256:
        raise CorpusV4AccuracyDatasetError("training split artifact hash mismatch")
    split_seed = task_row.get("split_seed")
    if type(split_seed) is not int or split_seed not in SPLIT_SEEDS:
        raise CorpusV4AccuracyDatasetError("task split seed is invalid")
    partitions = task_row.get("partitions")
    if not isinstance(partitions, dict) or set(partitions) != {
        "train",
        "validation",
        "test",
    }:
        raise CorpusV4AccuracyDatasetError("task partition table is malformed")
    expected_by_partition = {
        name: tuple(partitions[name]["layout_ids"])
        for name in ("train", "validation")
    }
    expected_ids = (*expected_by_partition["train"], *expected_by_partition["validation"])
    if len(expected_ids) != len(set(expected_ids)):
        raise CorpusV4AccuracyDatasetError("training membership is duplicated")
    if set(expected_ids) & set(partitions["test"]["layout_ids"]):
        raise CorpusV4AccuracyDatasetError("training membership intersects held-out IDs")
    ...
    expected_units = ("pF", "nH", "nH", "nH")
    samples: list[AccuracySample] = []
    observed_ids: list[int] = []
    for row in _load_jsonl(path):
        if set(row) != TRAINING_ROW_KEYS:
            raise CorpusV4AccuracyDatasetError("training row key set is not exact")
        if row.get("schema") != TRAINING_ROW_SCHEMA or row.get("split_seed") != split_seed:
            raise CorpusV4AccuracyDatasetError("training row schema or split differs")
        partition = row.get("partition")
        if partition not in expected_by_partition:
            raise CorpusV4AccuracyDatasetError("training row partition is not admitted")
        layout_id = row.get("layout_id")
        if type(layout_id) is not int or layout_id not in expected_by_partition[partition]:
            raise CorpusV4AccuracyDatasetError("training row layout membership differs")
        geometry = row.get("geometry_sha256")
        layout = row.get("layout")
        family_id = row.get("family_id")
        if (
            not isinstance(geometry, str)
            or not SHA256_RE.fullmatch(geometry)
            or not isinstance(layout, dict)
            or not isinstance(family_id, str)
            or family_id not in partitions[partition]["family_ids"]
        ):
            raise CorpusV4AccuracyDatasetError("training row identity is malformed")
        validate_layout(layout)
        if geometry_sha256(layout) != geometry:
            raise CorpusV4AccuracyDatasetError("training row geometry hash mismatch")
        if row.get("target_order") != list(TARGET_NAMES):
            raise CorpusV4AccuracyDatasetError("training target order differs")
        if tuple(row.get("target_fidelity_ids", ())) != expected_fidelity:
            raise CorpusV4AccuracyDatasetError("training target fidelity differs")
        if tuple(row.get("target_units", ())) != expected_units:
            raise CorpusV4AccuracyDatasetError("training target units differ")
        target_values = row.get("target_values")
        if not isinstance(target_values, list) or len(target_values) != len(TARGET_NAMES):
            raise CorpusV4AccuracyDatasetError("training target vector is malformed")
        values = tuple(
            _positive_float(value, name)
            for value, name in zip(target_values, TARGET_NAMES)
        )
        if values[3] > math.sqrt(values[1] * values[2]) * (1.0 + 1e-9):
            raise CorpusV4AccuracyDatasetError("training target violates passivity")
        ...
        samples.append(
            AccuracySample(
                ...
                l_sec_nh=values[2],
                l_mut_nh=values[3],
                r3_artifact_sha256=expected_sha256,
            )
        )
    if tuple(observed_ids) != expected_ids:
        raise CorpusV4AccuracyDatasetError(
            "training rows are not the exact ordered train/validation membership"
        )
    return TrainingSplitDataset(
        samples=tuple(samples),
        split_seed=split_seed,
        train_layout_ids=expected_by_partition["train"],
        validation_layout_ids=expected_by_partition["validation"],
        artifact_sha256=expected_sha256,
    )


def load_corpus_v4_accuracy_dataset_v3(
    ...
    expected_protocol_path = root / "protocols/corpus_v4_accuracy_v3.json"
    ...
    archive_result = verify_archive(paths["archive_manifest"])
    if (
        archive_result.get("status") != "PASS"
        or archive_result.get("dataset_admission_sha256")
        != protocol["inputs"]["dataset_admission"]["sha256"]
        or archive_result.get("result_files_verified") != 1698
        or archive_result.get("training_may_start") is not False
    ):
        raise CorpusV4AccuracyDatasetError(
            "upstream FEM-v2 archive verification did not close"
        )
