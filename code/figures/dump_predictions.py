...
Reproduces the exact 80/20 split from pipeline.py (np default_rng
seeded shuffle, split BEFORE any other rng use) and the exact normalizations
stored in the checkpoint. Runs via sbatch on a compute node (HARD RULE).
"""
import argparse
import json
...
def main():
    ...
    ap.add_argument("--ckpt", default="../results/run_v1/gnn_checkpoint.pt")
    ap.add_argument("--out", default="../results/run_v1/predictions.json")
