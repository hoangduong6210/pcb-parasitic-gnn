#!/usr/bin/env python3
"""
make_figures.py — generate the support figures for the IMPACT paper from REAL
run artifacts (results.json, scaling.json, predictions.json). Light matplotlib
only (no compute); writes both PDF (for LaTeX) and PNG to figures/.
"""
import json
...
def fig_parity():
    ...
    for k, ax in enumerate(axs):
        ...
        r2 = 1 - ss_res / ss_tot
        nice = {"Cps_pF": "$C_{ps}$", "L_pri_nH": "$L_p$", "L_sec_nH": "$L_s$", "L_mut_nH": "$M$"}.get(names[k], names[k])
