...
position-blind) is from the 3-D solver. SLURM only (HARD RULE).
"""
    ...
    try:
        ...
        for line in r.stdout.splitlines():
            if line.startswith('CPS='):
                return float(line[4:])
        # no CPS= line: surface the real reason instead of a silent skip
        print(f'[fem_cps] rc={r.returncode} stderr={r.stderr[:200]!r}', flush=True)
        return None
    except Exception as e:
        print(f'[fem_cps] exception {type(e).__name__}: {e}', flush=True)
        return None
    finally:
        try: os.remove(p)
...
def main():
    ...
    print("[femcps] FEM-3D C_ps + FastHenry L on %d layouts" % len(cand), flush=True)
    ...
    out = {"note": "all four targets field-grade: GNN vs FEM-3D (Cps) + FastHenry-3D (L)",
    ...
    src = {"Cps_pF": "fem3d_electrostatic", "L_pri_nH": "fasthenry_3d", "L_sec_nH": "fasthenry_3d", "L_mut_nH": "fasthenry_3d"}
    ...
    od = ROOT / "results" / "run_femcps"; od.mkdir(parents=True, exist_ok=True)
    (od / "results_femcps.json").write_text(json.dumps(out, indent=2))
