#!/usr/bin/env python3
"""
Lateral-aware ranking: vary BOTH interleaving (layer/z) AND lateral registration
(x-y overlap) - the layout levers a real optimiser turns. The analytical PEEC is
position-blind in x-y (overlap=min(L)*min(w)), so it should rank these poorly while
the geometry-correct GNN tracks the FEM truth. Honest map of where each tool helps.
Original decision-consequence demo: does the surrogate CHANGE the layout decision, or just
...
interleaving; a position-blind one picks wrong. Family-disjoint. SLURM (HARD RULE).
"""
...
def variants(base, n, rng):
    ...
    for _ in range(n):
        ...
        dx = float(rng.uniform(0.0, 25.0))   # lateral registration shift (x-y overlap = PEEC blind spot)
        for t in lay["traces"]:
            t["layer"] = int(perm[t["layer"] % 8]); t["z_mm"] = t["layer"] * 0.18 + 0.05
            if t.get("net") == "sec":
                t["x0"] = t.get("x0", 0.0) + dx
        out.append(lay)
    return out
...
def main():
    ...
    print("[ranklat] build families (layer + lateral registration): FEM-3D truth + analytical PEEC + graph", flush=True)
    fams = []; t0 = time.time()
    for b in bases:
        recs = []
    ...
    n = len(test_f)
    out = {"note": "lateral-aware ranking: GNN vs PEEC when variants vary x-y registration (PEEC blind spot) + interleaving",
    ...
    od = ROOT / "results" / "run_ranklat"; od.mkdir(parents=True, exist_ok=True)
    (od / "results_ranklat.json").write_text(json.dumps(out, indent=2))
