#!/usr/bin/env python3
"""SLURM-array FEM sensitivity study on nine geometry-valid v3 layouts."""
...
SCHEMA = "pcb-gnn.corpus-v3-reference-validation-task.v1"
SETTINGS = ((1, 8.0), (1, 12.0), (2, 8.0), (2, 12.0))
...
def selected_indices(samples: list[dict[str, Any]]) -> list[int]:
    ...
    if len(selected) != 9 or len(set(selected)) != 9:
        raise ValueError("stratified validation selection is not nine unique layouts")
    return selected
...
def solve(layout: dict[str, Any], refine: int, pad_mm: float, timeout_s: int) -> dict[str, Any]:
    ...
    result = json.loads(result_lines[0].removeprefix("RESULT="))
    return {
        "refine": refine,
        "pad_mm": pad_mm,
        "wall_ms": elapsed_ms,
...
def main() -> None:
    ...
    if args.validate_only:
        print(json.dumps({"schema": SCHEMA, "status": "validation-ok", "n_tasks": 9,
                          "settings": SETTINGS}))
        return
    if not os.environ.get("SLURM_ARRAY_JOB_ID") or not os.environ.get("SLURM_ARRAY_TASK_ID"):
        raise SystemExit("Submit the v3 reference validation as a SLURM array")
    if args.corpus is None:
        raise SystemExit("--corpus is required")
    task_id = int(os.environ["SLURM_ARRAY_TASK_ID"])
    if task_id not in range(9):
        raise SystemExit("canonical validation array has tasks 0..8")
    dirty = subprocess.run(
        ["git", "status", "--short", "--untracked-files=no"], cwd=ROOT,
        capture_output=True, text=True, check=True,
    ).stdout.splitlines()
    if dirty:
        raise SystemExit("Refusing reference validation from a dirty tracked worktree")
    ...
    for refine, pad_mm in SETTINGS:
        row = solve(sample["layout"], refine, pad_mm, args.timeout_s)
        rows.append(row)
        print(
            f"layout={sample['layout_id']:04d} refine={refine} pad={pad_mm:g} "
            f"Cps={row['cps_pf']:.6f}pF wall={row['wall_ms'] / 1000:.2f}s",
            flush=True,
        )

    worker = fem_cps_diagnostic_worker_path()
    sources = [
        Path(__file__), CODE / "data/v3_dataset.py", CODE / "core/geometry_contract.py",
        CODE / "core/planar_to_graph.py", CODE / "solvers/fem_capacitance_3d.py", worker,
    ]
    output = ROOT / "results/corpus_v3/reference_validation/jobs" / (
    ...
    result = {
        ...
        "selection": {
            "rule": "trace-count terciles crossed with low/median/high solver Cps",
            ...
            "published_reference_cps_pf": float(sample["y"][0]),
        },
        "settings": rows,
    }
