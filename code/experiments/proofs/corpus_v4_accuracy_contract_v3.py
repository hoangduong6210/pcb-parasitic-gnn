...
PROTOCOL_SCHEMA = "pcb-gnn.corpus-v4-accuracy-protocol.v3"
...
PLAN_ARTIFACT_NAMES = frozenset(
    {"evaluation_dataset.jsonl", "task_manifest.jsonl", *TRAINING_ARTIFACT_NAMES}
)
...
EXECUTION_SOURCE_NAMES = (
    ...
    "code/env.sh",
    "code/experiments/proofs/build_corpus_v4_accuracy_execution_lock_v3.py",
    "code/experiments/proofs/corpus_v4_accuracy_contract_v3.py",
    "code/experiments/proofs/finalize_corpus_v4_accuracy_v3.py",
    "code/experiments/proofs/plan_corpus_v4_accuracy_v3.py",
    "code/experiments/proofs/plan_corpus_v4_accuracy_resume_v3.py",
...
FINALIZER_SCHEDULER_RECORD_ALLOWLIST = tuple(
    name
    for name in SCHEDULER_RECORD_ALLOWLIST
    if name not in {"ArrayJobId", "ArrayTaskId", "ArrayTaskThrottle"}
)
...
TERMINAL_RECEIPT_KEYS = frozenset(
    {"AllocTRES", "ElapsedRaw", "ExitCode", "JobID", "JobIDRaw", "ReqTRES", "State"}
)
...
def validate_protocol(path: Path, expected_sha256: str | None = None) -> tuple[dict[str, Any], str]:
    ...
    expected_sections = {
        ...
        "forbidden_actions": {
            ...
            "full_dataset_loader_in_training": True,
            "global_r3_to_r4_correction": True,
            "heldout_bytes_in_training_namespace": True,
        ...
        "sandbox": {
            ...
            "filesystem_namespaces": ["user", "mount", "pid", "ipc", "uts"],
            "network_namespace": (
                "shared because the site forbids loopback setup in an unprivileged namespace"
            ),
            "python_site_packages_mount": "/opt/pcb-python/site-packages",
            "required": True,
    ...
    if protocol.get("protocol_name") != "corpus-v4-fem-v2-family-crossed-accuracy-v3":
        raise ValueError("accuracy protocol name differs from the frozen v3 identity")
...
def validate_plan(
    ...
    *,
    ...
    if (
        ...
        or set(artifacts) != PLAN_ARTIFACT_NAMES
    ...
    for task_id, row in enumerate(rows):
        ...
        expected_training_name = f"training_split_{row['split_seed']}.jsonl"
        if (
            not isinstance(training_dataset, dict)
            or set(training_dataset) != {"path", "sha256"}
            or training_dataset.get("path") != expected_training_name
            or training_dataset.get("sha256") != artifacts.get(expected_training_name)
        ):
            raise ValueError(f"task row {task_id} training artifact pin is malformed")
...
def validate_root_closure(
    *,
    ...
    for name in TRAINING_ARTIFACT_NAMES:
        training_path = plan_path.parent / name
        if training_path.is_symlink() or not training_path.is_file():
            raise ValueError(f"training dataset must be a regular file: {name}")
        require_file_sha256(training_path, artifacts[name], f"training dataset {name}")
    ...
    if lock is not None and lock_sha256 is not None:
        ...
        expected_lock_roots = {
            ...
            "sandbox_executable_sha256": protocol["sandbox"]["executable_sha256"],
        ...
        if lock.get("training_artifact_sha256") != {
            name: artifacts[name] for name in TRAINING_ARTIFACT_NAMES
        }:
            raise ValueError("execution lock does not bind all training artifacts")
    return evaluation_sha256


def validate_training_root_closure(
    *,
    protocol: Mapping[str, Any],
    protocol_sha256: str,
    plan: Mapping[str, Any],
    ...
    """Validate one training root without opening any held-out artifact."""
    if plan.get("protocol_sha256") != protocol_sha256:
        raise ValueError("training plan does not bind the protocol root")
    if plan.get("input_sha256") != {
        name: protocol["inputs"][name]["sha256"] for name in sorted(EXPECTED_INPUT_NAMES)
    }:
        raise ValueError("training plan input commitments differ from the protocol")
    artifacts = plan.get("artifact_sha256")
    if not isinstance(artifacts, dict) or set(artifacts) != PLAN_ARTIFACT_NAMES:
        raise ValueError("training plan artifact commitment set is not exact")
    if artifacts.get("task_manifest.jsonl") != task_manifest_sha256:
        raise ValueError("training plan task-manifest commitment differs")
    training_pin = task_row.get("training_dataset")
    if not isinstance(training_pin, dict) or set(training_pin) != {"path", "sha256"}:
        raise ValueError("training dataset pin is malformed")
    training_name = training_pin["path"]
    if training_name != f"training_split_{task_row['split_seed']}.jsonl":
        raise ValueError("training dataset path differs from the task split")
    if artifacts.get(training_name) != training_pin["sha256"]:
        raise ValueError("training dataset commitment differs from the plan")
    training_path = plan_path.parent / training_name
    if training_path.is_symlink() or not training_path.is_file():
        raise ValueError("training dataset must be a regular non-symlink file")
    require_file_sha256(training_path, training_pin["sha256"], "training dataset")
    planner_sources = plan.get("planner_source_sha256")
    if not isinstance(planner_sources, dict) or set(planner_sources) != set(
        PLANNER_SOURCE_NAMES
    ):
        raise ValueError("training planner-source closure is not exact")
    for name in PLANNER_SOURCE_NAMES:
        require_file_sha256(resolve_repo_path(name, "planner source"), planner_sources[name], name)
    expected_lock_roots = {
        "heldout_evaluation_sha256": artifacts["evaluation_dataset.jsonl"],
        "plan_sha256": sha256_file(plan_path),
        "protocol_sha256": protocol_sha256,
        "sandbox_executable_sha256": protocol["sandbox"]["executable_sha256"],
        "task_manifest_sha256": task_manifest_sha256,
        "upstream_archive_sha256": protocol["inputs"]["archive_manifest"]["sha256"],
        "upstream_dataset_admission_sha256": protocol["inputs"]["dataset_admission"]["sha256"],
    }
    if not isinstance(lock_sha256, str) or SHA256_RE.fullmatch(lock_sha256) is None:
        raise ValueError("training execution-lock root is malformed")
    if any(lock.get(name) != digest for name, digest in expected_lock_roots.items()):
        raise ValueError("training execution lock differs from the frozen roots")
    if lock.get("training_artifact_sha256") != {
        name: artifacts[name] for name in TRAINING_ARTIFACT_NAMES
    }:
        raise ValueError("training execution lock differs from training commitments")
    return training_pin["sha256"]
...
def validate_execution_lock(path: Path, expected_sha256: str) -> tuple[dict[str, Any], str]:
    ...
    training_artifacts = lock.get("training_artifact_sha256")
    if (
        not isinstance(training_artifacts, dict)
        or set(training_artifacts) != set(TRAINING_ARTIFACT_NAMES)
        or any(
            not isinstance(value, str) or SHA256_RE.fullmatch(value) is None
            for value in training_artifacts.values()
        )
    ):
...
def _canonical_decimal(value: Any, label: str, *, allow_zero: bool = False) -> str:
    ...
    if int(value) < (0 if allow_zero else 1):
        raise ValueError(f"{label} is outside the admitted range")
    return value


def validate_scheduler_receipt(
    scheduler: Mapping[str, Any], *, stage: str, task_id: int | None = None
) -> None:
    """Validate exact in-run scheduler identity across environment and scontrol."""
    if stage not in {"training", "finalizer"} or set(scheduler) != SCHEDULER_RECEIPT_KEYS:
        raise ValueError("scheduler receipt schema is not exact")
    job_id = _canonical_decimal(scheduler.get("job_id"), "scheduler job ID")
    record = scheduler.get("scheduler_record")
    expected_record_keys = (
        set(SCHEDULER_RECORD_ALLOWLIST)
        if stage == "training"
        else set(FINALIZER_SCHEDULER_RECORD_ALLOWLIST)
    )
    if not isinstance(record, Mapping) or set(record) != expected_record_keys:
        raise ValueError("scontrol scheduler-record schema is not exact")
    if _canonical_decimal(record.get("JobId"), "scontrol JobId") != job_id:
        raise ValueError("scontrol JobId differs from the in-run job ID")
    if stage == "training":
        task_id = require_plain_int(task_id, "scheduler task ID")
        if scheduler.get("array_task_id") != task_id:
            raise ValueError("scheduler array task identity differs")
        array_job_id = _canonical_decimal(
            scheduler.get("array_job_id"), "scheduler array job ID"
        )
        if (
            record.get("ArrayJobId") != array_job_id
            or record.get("ArrayTaskId") != str(task_id)
        ):
            raise ValueError("scontrol array identity differs from the in-run receipt")
    elif scheduler.get("array_job_id") is not None or scheduler.get("array_task_id") is not None:
        raise ValueError("finalizer scheduler receipt must be non-array")


def validate_terminal_receipt(
    scheduler: Mapping[str, Any], terminal: Mapping[str, Any], *, stage: str
) -> dict[str, str]:
    """Close one scontrol receipt to its exact terminal sacct identity."""
    task_id = scheduler.get("array_task_id") if stage == "training" else None
    validate_scheduler_receipt(scheduler, stage=stage, task_id=task_id)
    if set(terminal) != TERMINAL_RECEIPT_KEYS:
        raise ValueError("terminal scheduler receipt schema is not exact")
    job_id = _canonical_decimal(scheduler["job_id"], "scheduler job ID")
    raw = _canonical_decimal(terminal.get("JobIDRaw"), "terminal JobIDRaw")
    if raw != job_id or scheduler["scheduler_record"]["JobId"] != raw:
        raise ValueError("terminal raw job identity differs from the in-run receipt")
    expected_logical = (
        f"{scheduler['array_job_id']}_{task_id}" if stage == "training" else job_id
    )
    if terminal.get("JobID") != expected_logical:
        raise ValueError("terminal logical job identity differs")
    if terminal.get("State") != "COMPLETED" or terminal.get("ExitCode") != "0:0":
        raise ValueError("terminal scheduler state is not successful")
    _canonical_decimal(terminal.get("ElapsedRaw"), "terminal elapsed seconds", allow_zero=True)
    record = scheduler["scheduler_record"]
    if any(
        not tres_equivalent(terminal.get(name), record.get(name))
        for name in ("ReqTRES", "AllocTRES")
    ):
        raise ValueError("terminal resources differ from the in-run receipt")
    return {
        **{name: str(terminal[name]) for name in TERMINAL_RECEIPT_KEYS},
        "ReqTRES": canonical_tres(terminal["ReqTRES"]),
        "AllocTRES": canonical_tres(terminal["AllocTRES"]),
    }
...
def validate_slurm_allocation(
    ...
    *,
    ...
    record_names = (
        SCHEDULER_RECORD_ALLOWLIST
        if stage == "training"
        else FINALIZER_SCHEDULER_RECORD_ALLOWLIST
    )
    if any(name not in fields for name in record_names):
        raise SystemExit("active SLURM receipt lacks a required sanitized field")
    receipt = {
        **resources,
        "array_job_id": os.environ.get("SLURM_ARRAY_JOB_ID"),
        "array_task_id": task_id,
        "job_id": os.environ["SLURM_JOB_ID"],
        "scheduler_record": {name: fields[name] for name in record_names},
    }
    validate_scheduler_receipt(receipt, stage=stage, task_id=task_id)
    return receipt
...
def write_execution_lock(
    ...
    *,
    ...
    payload = {
        ...
        "heldout_evaluation_sha256": plan["artifact_sha256"][
        ...
        "task_manifest_sha256": task_manifest_sha256,
        "training_artifact_sha256": {
            name: plan["artifact_sha256"][name] for name in TRAINING_ARTIFACT_NAMES
        },
        "upstream_archive_sha256": protocol["inputs"]["archive_manifest"]["sha256"],
