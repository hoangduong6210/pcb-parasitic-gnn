...
from verify_corpus_v4_fem_v2_production_archive import verify_archive  # noqa: E402


PROTOCOL_SCHEMA = "pcb-gnn.corpus-v4-accuracy-protocol.v3"
PLAN_SCHEMA = "pcb-gnn.corpus-v4-accuracy-plan.v3"
DATASET_ROW_SCHEMA = "pcb-gnn.corpus-v4-accuracy-evaluation-row.v3"
TRAINING_ROW_SCHEMA = "pcb-gnn.corpus-v4-accuracy-training-row.v3"
TASK_SCHEMA = "pcb-gnn.corpus-v4-accuracy-task-row.v3"
DEFAULT_PROTOCOL = ROOT / "protocols/corpus_v4_accuracy_v3.json"
DEFAULT_OUT = ROOT / "results/corpus_v4/accuracy_v3/plan/v1"
TRAINING_ARTIFACT_NAMES = tuple(
    f"training_split_{seed}.jsonl" for seed in (40, 41, 42, 43, 44)
)
CONTROL_ARTIFACT_NAMES = (
    "evaluation_dataset.jsonl",
    "task_manifest.jsonl",
    "plan.json",
)
ARTIFACT_NAMES = (*TRAINING_ARTIFACT_NAMES, *CONTROL_ARTIFACT_NAMES)
...
def validate_protocol(protocol: dict[str, Any]) -> None:
    ...
    if protocol.get("protocol_name") != "corpus-v4-fem-v2-family-crossed-accuracy-v3":
        raise AccuracyPlanError("unexpected accuracy-v3 protocol identity")
    if protocol.get("upstream_archive_git_commit") != (
        "d512cf242a5b2b8faf60de9c6bba0a814f867ec6"
    ):
        raise AccuracyPlanError("accuracy-v3 archive commit is not frozen")
    ...
    if protocol.get("sandbox") != {
        ...
        "network_namespace": "shared because the site forbids loopback setup in an unprivileged namespace",
        "python_site_packages_mount": "/opt/pcb-python/site-packages",
        "required": True,
        "sandbox_root": "/workspace",
    }:
        raise AccuracyPlanError("training sandbox policy differs from the frozen boundary")
...
def validate_archive_links(
    ...
    if archive.get("schema") != "pcb-gnn.corpus-v4-fem-v2-production-archive.v1":
        raise AccuracyPlanError("unexpected upstream archive schema")
    links = (
        (
            archive["source_corpus"]["summary"],
            protocol["inputs"]["source_corpus_summary"],
        ),
        (archive["source_corpus"]["layouts"], protocol["inputs"]["layouts"]),
        (
            archive["source_corpus"]["inductance_labels"],
            protocol["inputs"]["inductance_labels"],
        ),
        (
            archive["source_corpus"]["family_registry"],
            protocol["inputs"]["geometry_families"],
        ),
        (
            archive["source_corpus"]["hf_selection_registry"],
            protocol["inputs"]["hf_selection_registry"],
        ),
        (
            archive["source_corpus"]["split_registry"],
            protocol["inputs"]["split_registry"],
        ),
        (archive["dataset"]["admission"], protocol["inputs"]["dataset_admission"]),
        (
            archive["dataset"]["observations"],
            protocol["inputs"]["final_observations"],
        ),
    )
    ...
    admission = load_json(paths["dataset_admission"])
    if (
        ...
        raise AccuracyPlanError("upstream dataset admission boundary is invalid")
    verified = verify_archive(paths["archive_manifest"])
    if (
        verified.get("status") != "PASS"
        or verified.get("dataset_admission_sha256")
        != protocol["inputs"]["dataset_admission"]["sha256"]
        or verified.get("result_files_verified") != 1698
    ):
        raise AccuracyPlanError("upstream FEM-v2 archive verifier did not close")


def validate_upstream_git_commit(protocol: dict[str, Any]) -> None:
    commit = protocol["upstream_archive_git_commit"]
    if subprocess.run(
        ...
        raise AccuracyPlanError("upstream archive commit is not an ancestor")
    for name, record in sorted(protocol["inputs"].items()):
        blob = subprocess.run(
        ...
        if blob.returncode != 0 or sha256_bytes(blob.stdout) != record["sha256"]:
            raise AccuracyPlanError(
                f"protocol input is not the pinned upstream Git blob: {name}"
            )
...
def validate_observations(
    ...
    for row in rows:
        ...
        if (
            ...
            or not isinstance(layout_id, int)
            or layout_id not in layouts
            or fidelity_id not in {"cps_fem_r3_p16_t1_v2", "cps_fem_r4_p16_t1_v2"}
            or key in by_key
            or row.get("geometry_sha256") != layouts[layout_id]["geometry_sha256"]
            or row.get("units") != "pF"
        ):
        ...
        by_key[key] = row
    r3 = {
        layout_id: float(by_key[(layout_id, "cps_fem_r3_p16_t1_v2")]["cps_pf"])
        for layout_id in range(1500)
        if (layout_id, "cps_fem_r3_p16_t1_v2") in by_key
    }
    r4 = {
        layout_id: float(by_key[(layout_id, "cps_fem_r4_p16_t1_v2")]["cps_pf"])
        for layout_id in selected
        if (layout_id, "cps_fem_r4_p16_t1_v2") in by_key
    }
...
def build_training_rows(
    ...
    """Create one label-isolated train/validation artifact for a split.

    The artifact deliberately contains neither test layouts nor the sparse R4
    comparator.  Training tasks consume this artifact instead of reopening the
    global solver observations.
    """
    split_seed = int(split["split_seed"])
    evaluation_by_id = {row["layout_id"]: row for row in evaluation_rows}
    rows: list[dict[str, Any]] = []
    admitted_ids: set[int] = set()
    for partition in ("train", "validation"):
        for layout_id in split["partitions"][partition]["layout_ids"]:
            if layout_id in admitted_ids:
                raise AccuracyPlanError("training artifact membership is duplicated")
            admitted_ids.add(layout_id)
            evaluation = evaluation_by_id[layout_id]
            references = evaluation["training_reference"]
            rows.append(
                {
                    ...
                    "target_fidelity_ids": [
                        references[name]["fidelity_id"] for name in TARGET_NAMES
                    ],
                    "target_order": list(TARGET_NAMES),
                    "target_units": [
                        references[name]["units"] for name in TARGET_NAMES
                    ],
                    "target_values": [
                        references[name]["value"] for name in TARGET_NAMES
                    ],
                }
            )
    expected_ids = set(split["partitions"]["train"]["layout_ids"]) | set(
        split["partitions"]["validation"]["layout_ids"]
    )
    test_ids = set(split["partitions"]["test"]["layout_ids"])
    if admitted_ids != expected_ids or admitted_ids & test_ids:
        raise AccuracyPlanError("training artifact is not isolated from test membership")
    return rows
            ...
            tasks.append(
                {
                    ...
                    "task_id": task_id,
                    "training_dataset": {
                        "path": f"training_split_{split_seed}.jsonl",
                        "sha256": training_sha256[split_seed],
                    },
                }
            )
...
def build_artifacts(
    *, root: Path = ROOT, protocol_path: Path = DEFAULT_PROTOCOL
    ...
    validate_upstream_git_commit(protocol)
    ...
    training_bytes = {
        split["split_seed"]: canonical_jsonl_bytes(
            build_training_rows(split, layouts, evaluation_rows)
        )
    ...
    task_rows = build_task_rows(
        ...
        {seed: sha256_bytes(payload) for seed, payload in training_bytes.items()},
        split_rows,
        layouts,
    )
    ...
    plan = {
        "artifact_sha256": {
            ...
            **{
                f"training_split_{seed}.jsonl": sha256_bytes(payload)
                for seed, payload in sorted(training_bytes.items())
            },
        },
        "counts": {
            ...
            "training_rows_by_split": {
                str(seed): len(
                    split["partitions"]["train"]["layout_ids"]
                    + split["partitions"]["validation"]["layout_ids"]
                )
                for seed, split in sorted(
                    (row["split_seed"], row) for row in split_rows
                )
            },
        },
        ...
        "planner_source_sha256": {
            ...
            "code/experiments/proofs/plan_corpus_v4_accuracy_v3.py": sha256_file(
                Path(__file__).resolve()
            ),
            "code/quality/verify_corpus_v4_fem_v2_production_archive.py": sha256_file(
                root
    ...
    return {
        **{
            f"training_split_{seed}.jsonl": payload
            for seed, payload in sorted(training_bytes.items())
        },
