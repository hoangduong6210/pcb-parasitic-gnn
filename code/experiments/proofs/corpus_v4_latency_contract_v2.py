...
from corpus_v4_accuracy_contract_v3 import (  # noqa: E402
    ...
    resolve_repo_path,
    tres_equivalent,
    validate_historical_execution_lock as validate_accuracy_execution_lock,
...
PLAN_SCHEMA = f"{SCHEMA_PREFIX}-plan.v1"
PANEL_ROW_SCHEMA = f"{SCHEMA_PREFIX}-panel-row.v1"
TASK_ROW_SCHEMA = f"{SCHEMA_PREFIX}-task-row.v1"
LOCK_SCHEMA = f"{SCHEMA_PREFIX}-execution-lock.v1"
TASK_MANIFEST_SCHEMA = (
    f"{SCHEMA_PREFIX}-task-artifact-manifest.v1"
)
TASK_RESULT_SCHEMA = f"{SCHEMA_PREFIX}-task.v1"
PREFLIGHT_ADMISSION_SCHEMA = (
    f"{SCHEMA_PREFIX}-preflight-admission.v1"
)
FEM_V2_DATASET_ADMISSION_SCHEMA = (
    "pcb-gnn.corpus-v4-fem-v2-production-dataset-admission.v1"
)
PENDING_SCHEMA = f"{SCHEMA_PREFIX}-pending-set.v1"
...
EXPECTED_INPUT_PATHS = {
    "accuracy_protocol": "protocols/corpus_v4_accuracy_v3.json",
    "accuracy_archive": "results/corpus_v4/accuracy_v3/ARCHIVE_MANIFEST.json",
    "accuracy_accepted_set": (
        "results/corpus_v4/accuracy_v3/resume/round_01/accepted_artifact_set.json"
    ),
    "accuracy_plan": "results/corpus_v4/accuracy_v3/plan/v1/plan.json",
    ...
    "fem_v2_production_protocol": "protocols/corpus_v4_fem_v2_production_v1.json",
    "fem_v2_production_lock": "protocols/corpus_v4_fem_v2_production_lock_v1.json",
...
PLANNER_SOURCE_NAMES = (
    ...
    "code/data/corpus_v4_accuracy_dataset_v3.py",
    "code/experiments/proofs/corpus_v4_accuracy_contract_v3.py",
...
EXECUTION_SOURCE_NAMES = (
    ...
    "code/env.sh",
    "code/experiments/proofs/build_corpus_v4_latency_execution_lock_v2.py",
    "code/experiments/proofs/admit_corpus_v4_latency_preflight_v2.py",
    "code/experiments/proofs/corpus_v4_accuracy_contract_v3.py",
    "code/experiments/proofs/corpus_v4_fem_v2_production.py",
    "code/experiments/proofs/corpus_v4_latency_contract_v2.py",
    ...
    "code/experiments/proofs/plan_corpus_v4_latency_v2.py",
    "code/experiments/proofs/plan_corpus_v4_latency_resume_v2.py",
    "code/experiments/proofs/run_corpus_v4_cps_multifidelity_task.py",
    "code/experiments/proofs/run_corpus_v4_accuracy_task_v3.py",
...
def validate_protocol(
    ...
    if path.resolve() != (ROOT / "protocols/corpus_v4_latency_fem_v2_v1.json").resolve():
    ...
    if (
        ...
        != "corpus-v4-fem-v2-paired-latency-v1"
    ):
    ...
    _exact_json(
        protocol.get("solver_workflow"),
        {
            ...
            "comparison_scope": (
                "sequential FastHenry-plus-one-thread-FEM-v2-R3P16 workflow used "
                "to obtain all four targets"
            ),
            "execution_order": ["FastHenry", "FEM-v2-R3P16-one-thread"],
            "inductance": {
                "binary_sha256": (
            ...
            "sequential": True,
            "sequential_outer_wall_timer": True,
    ...
    _exact_json(
        protocol.get("gnn_timing"),
        {
            ...
            "model_only_supporting": {
                ...
                "measured_repetitions": 200,
                "role": (
                    "secondary diagnostic; never the denominator of the "
                    "solver-workflow speedup"
                ),
                "timer": "time.perf_counter_ns",
    ...
    _exact_json(
        protocol.get("resources"),
        {
            ...
            "full_array": {
                ...
                "memory_gib": 48,
                "partition": "nextgen",
                "scientific_threads": {"blas": 1, "gmsh": 1, "torch": 1},
                "time_limit": "02:00:00",
            },
            "preflight": {
                "allocated_cpus_may_exceed_request": True,
    ...
    _exact_json(
        protocol.get("statistics"),
        {
            ...
            "design_solver_latency": (
                "one sequential FastHenry-plus-one-thread-FEM-v2-R3P16 observation"
            ),
            "primary_estimand": (
            ...
            "secondary_estimands": [
                "median over 306 designs of FastHenry latency divided by design-median GNN latency",
                "median over 306 designs of one-thread FEM-v2 latency divided by design-median GNN latency",
            ],
        },
        "latency statistics",
    )
...
def validate_upstream_closure(
    ...
    """Bind the FEM-v2 latency study to admitted dataset and accuracy roots."""
    ...
    if set(paths) != EXPECTED_INPUT_NAMES or any(
        ...
        raise LatencyContractError("FEM-v2 latency input closure is not canonical")
    ...
    _, training_lock_sha = validate_accuracy_execution_lock(
        paths["accuracy_training_execution_lock"],
        protocol["inputs"]["accuracy_training_execution_lock"]["sha256"],
        expected_source_git_head="c0ffca0d0637e8fbba81c126c3f56f8316003a9a",
    )
    if (
        accuracy_plan["artifact_sha256"].get("evaluation_dataset.jsonl")
        != protocol["inputs"]["evaluation_dataset"]["sha256"]
    ):
        raise LatencyContractError("evaluation dataset differs from accuracy-v3 plan")

    archive = load_json(paths["accuracy_archive"])
    expected_accuracy_bindings = {
        "accepted_set_sha256": protocol["inputs"]["accuracy_accepted_set"]["sha256"],
        "finalizer_execution_lock_sha256": protocol["inputs"]["accuracy_finalizer_execution_lock"]["sha256"],
        "finalizer_source_git_head": "cdd3a7555377360208e1c582b6e94aca8fbfdd60",
        "plan_sha256": accuracy_plan_sha,
        "protocol_sha256": accuracy_protocol_sha,
        "task_manifest_sha256": accuracy_tasks_sha,
        "training_execution_lock_sha256": training_lock_sha,
        "training_source_git_head": "c0ffca0d0637e8fbba81c126c3f56f8316003a9a",
    }
    verified = archive.get("verified_analysis_files_sha256", {})
    if (
        archive.get("schema") != "pcb-gnn.corpus-v4-accuracy-archive.v4"
        or archive.get("bindings") != expected_accuracy_bindings
        or archive.get("analysis_manifest", {}).get("sha256")
        != protocol["inputs"]["accuracy_analysis_manifest"]["sha256"]
        or verified.get("summary.json")
        ...
        raise LatencyContractError("accuracy-v3 archive bindings are not exact")

    accepted = load_json(paths["accuracy_accepted_set"])
    accepted_entries = accepted.get("entries")
    if (
        accepted.get("schema") != "pcb-gnn.corpus-v4-accuracy-accepted-set.v3"
        ...
        or len(accepted_entries) != 25
    ):
        raise LatencyContractError("accuracy-v3 accepted checkpoint set is incomplete")
    accepted_entry = accepted_entries[DESIGNATED_TASK_ID]
    if (
        ...
        raise LatencyContractError("designated accuracy-v3 checkpoint was not accepted")
    ...
    if any(
        manifest["files_sha256"].get(artifact_name)
        != protocol["inputs"][input_name]["sha256"]
        for artifact_name, input_name in required_task_files.items()
    ):
        raise LatencyContractError("task-12 manifest does not bind the latency inputs")
    ...
    if (
        result.get("schema") != "pcb-gnn.corpus-v4-accuracy-task-result.v3"
        or result.get("task") != expected_task_identity
        ...
        or result.get("checkpoint") != expected_checkpoint
    ):
        raise LatencyContractError("task-12 result identity is not exact")

    summary = load_json(paths["accuracy_final_summary"])
    checkpoint_identity = summary.get("designated_downstream_checkpoint")
    if (
        summary.get("schema") != "pcb-gnn.corpus-v4-accuracy-final.v4"
        or not isinstance(checkpoint_identity, dict)
        or any(
            checkpoint_identity.get(name) != value
            for name, value in {
                **expected_task_identity,
                **expected_checkpoint,
            }.items()
        ...
        raise LatencyContractError("accuracy-v3 summary does not bind task 12")
    ...
    if (
        ...
        != "pcb-gnn.corpus-v4-accuracy-checkpoint-index.v3"
        ...
        raise LatencyContractError("accuracy-v3 checkpoint index differs")
    ...
    if (
        ...
        or any(payload.get(name) != value for name, value in expected_task_identity.items())
    ):
        raise LatencyContractError("task-12 safe-bundle metadata differs")

    fem_protocol = load_json(paths["fem_v2_production_protocol"])
    fidelity = fem_protocol.get("fidelities", {}).get("cps_fem_r3_p16_t1_v2")
    threads = fem_protocol.get("runtime", {}).get("threads")
    if (
        fem_protocol.get("schema")
        != "pcb-gnn.corpus-v4-fem-v2-production-protocol.v1"
        or fem_protocol.get("protocol_name")
        != "corpus-v4-fem-v2-one-thread-production-v1"
        or fidelity
        ...
        raise LatencyContractError("FEM-v2 one-thread production protocol differs")

    dataset_admission = load_json(paths["fem_v2_dataset_admission"])
    if (
        dataset_admission.get("schema") != FEM_V2_DATASET_ADMISSION_SCHEMA
        or dataset_admission.get("admission_eligible") is not True
        or dataset_admission.get("solver_executed") is not False
        or dataset_admission.get("decision")
        != {
            "accuracy_protocol_may_be_frozen": True,
            "dataset_generation_admitted": True,
            "training_may_start": False,
        }
        or dataset_admission.get("preterminal_files", {}).get("label_observations_sha256")
        != protocol["inputs"]["fem_v2_final_observations"]["sha256"]
    ):
        raise LatencyContractError("FEM-v2 dataset admission is not positive")
    dataset_archive = load_json(paths["fem_v2_dataset_archive"])
    if (
        dataset_archive.get("schema")
        != "pcb-gnn.corpus-v4-fem-v2-production-archive.v1"
        or dataset_archive.get("frozen_roots", {}).get("protocol", {}).get("sha256")
        != protocol["inputs"]["fem_v2_production_protocol"]["sha256"]
        or dataset_archive.get("frozen_roots", {}).get("execution_lock", {}).get("sha256")
        != protocol["inputs"]["fem_v2_production_lock"]["sha256"]
        or dataset_archive.get("dataset", {}).get("admission", {}).get("sha256")
        != protocol["inputs"]["fem_v2_dataset_admission"]["sha256"]
        or dataset_archive.get("dataset", {}).get("observations", {}).get("sha256")
        != protocol["inputs"]["fem_v2_final_observations"]["sha256"]
        or dataset_archive.get("dataset", {}).get("counts")
        != {
            "geometries": 1500,
        ...
        raise LatencyContractError("FEM-v2 dataset archive bindings differ")
    ...
    if (
        ...
        raise LatencyContractError("designated test partition is not split-42 task 12")
...
def validate_root_closure(
    *,
    ...
    dataset = load_corpus_v4_accuracy_dataset_v3(
        root=ROOT,
...
def validate_fem_v2_dataset_admission(
    path: Path,
    expected_sha256: str,
) -> dict[str, Any]:
    """Authenticate the frozen positive FEM-v2 dataset-admission root."""
    _require_sha256(expected_sha256, "FEM-v2 dataset admission SHA-256")
    expected_path = resolve_repo_path(
        EXPECTED_INPUT_PATHS["fem_v2_dataset_admission"],
        "FEM-v2 dataset admission",
    )
    if path.resolve() != expected_path.resolve():
        raise LatencyContractError("FEM-v2 dataset admission path is not canonical")
    if path.is_symlink() or not path.is_file():
        raise LatencyContractError("FEM-v2 dataset admission must be a regular file")
    require_file_sha256(path, expected_sha256, "FEM-v2 dataset admission")
    payload = load_json(path)
    if (
        payload.get("schema") != FEM_V2_DATASET_ADMISSION_SCHEMA
        or payload.get("admission_eligible") is not True
        or payload.get("solver_executed") is not False
...
def validate_preflight_admission_payload(
    ...
    *,
    ...
    validator_path = "code/experiments/proofs/admit_corpus_v4_latency_preflight_v2.py"
    if (
    ...
    dataset_reference = payload.get("fem_v2_dataset_admission")
    if (
        not isinstance(dataset_reference, Mapping)
        or set(dataset_reference) != {"path", "sha256"}
    ):
        raise LatencyContractError("FEM-v2 dataset admission reference is not exact")
    protocol_path = "protocols/corpus_v4_latency_fem_v2_v1.json"
    if source_hashes.get(protocol_path) != sha256_file(ROOT / protocol_path):
        raise LatencyContractError("FEM-v2 latency protocol is not source-locked")
    latency_protocol, _ = validate_protocol(ROOT / protocol_path)
    expected_dataset_reference = latency_protocol["inputs"]["fem_v2_dataset_admission"]
    if dict(dataset_reference) != expected_dataset_reference:
        raise LatencyContractError("FEM-v2 dataset admission pin differs from protocol")
    dataset_path = resolve_descendant_path(
        ROOT,
        dataset_reference["path"],
        "FEM-v2 dataset admission",
    )
    validate_fem_v2_dataset_admission(
        dataset_path,
        dataset_reference["sha256"],
    )
    if require_live_fem_accounting:
        raise LatencyContractError(
            "live FEM accounting is not part of the frozen dataset-admission contract"
        )
    ...
    from finalize_corpus_v4_latency_v2 import (  # noqa: PLC0415
        load_expected_references,
        validate_timing_record,
    )

    references = load_expected_references(latency_protocol)
    admitted_r3 = {
        row["layout_id"]: {
        ...
        for row in load_jsonl(
            resolve_repo_path(
                latency_protocol["inputs"]["fem_v2_final_observations"]["path"],
                "preflight FEM-v2 observations",
            )
        )
        if row.get("fidelity_id") == "cps_fem_r3_p16_t1_v2"
    }
    if len(admitted_r3) != 1500:
        raise LatencyContractError("preflight FEM-v2 observation panel is incomplete")

    failure_root = (
        ROOT
        / "results/corpus_v4/latency_fem_v2/preflight/failures"
        / f"job_{array_job_id}"
    )
    ...
    attempt_root = (
        ROOT
        / "results/corpus_v4/latency_fem_v2/preflight/attempts"
        / f"job_{array_job_id}"
    )
    ...
    expected_result_bindings = {
        **bindings,
        ...
        "fem_v2_dataset_admission": dict(dataset_reference),
    ...
    for expected_task_id, entry in zip(PREFLIGHT_TASK_IDS, entries):
        ...
        validate_timing_record(
            record,
            expected_repetitions=int(latency_protocol["gnn_timing"]["measured_repetitions"]),
            block_median_ratio_max=float(latency_protocol["gnn_timing"]["block_median_ratio_max"]),
        )
        if (
            any(
            ...
            or record.get("reference") != references[task["layout_id"]]
            or record.get("fem_v2_reference_identity")
            != admitted_r3.get(task["layout_id"])
            or float(record.get("solver_label_max_relative_drift", float("inf")))
            > float(latency_protocol["solver_workflow"]["reference_agreement_max_relative_error"])
        ):
            raise LatencyContractError("preflight numerical reference or task identity differs")
        ...
        if (
            ...
            != source_hashes.get("code/jobs/submit_corpus_v4_latency_preflight_v2.sh")
...
def validate_preflight_admission(
    ...
    *,
    ...
    expected_label = (
        "results/corpus_v4/latency_fem_v2/preflight/admission/"
    ...
    *,
    ...
    if is_array:
        ...
        valid = valid and (
            ...
            and os.environ["PCB_GNN_GMSH_THREADS"] == "1"
        )
