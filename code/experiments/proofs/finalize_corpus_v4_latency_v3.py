#!/usr/bin/env python3
"""Analysis-only recovery for admitted latency tasks with two immutable source roots."""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--finalizer-execution-lock", type=Path)
    parser.add_argument("--expected-finalizer-execution-lock-sha256")
    parser.add_argument("--expected-finalizer-source-git-head")
...
def _runtime_required(args: argparse.Namespace) -> None:
    names = (
        ...
        "finalizer_execution_lock", "expected_finalizer_execution_lock_sha256",
...
def canonical_output_directory(output_root: Path, job_id: str) -> Path:
    """Resolve the canonical archive before writing, including relative CLI paths."""
    if not isinstance(job_id, str) or not job_id or any(character not in "0123456789" for character in job_id):
        raise ValueError("finalizer job ID must contain only decimal digits")
    resolved_root = output_root.resolve()
    if resolved_root != (ROOT / "results/corpus_v4/latency_fem_v2/final").resolve():
        raise ValueError("recovery output root must be the canonical final archive")
    return resolved_root / f"job_{job_id}"
...
def main() -> None:
    ...
    finalizer_source_sha256 = validate_finalizer_source(
        finalizer_lock, args.expected_finalizer_source_git_head
    )
    finalizer_runtime = finalizer_runtime_identity(protocol)
    executed_batch = Path(os.environ.get("PCB_GNN_EXECUTED_BATCH_SCRIPT", ""))
    expected_batch = "code/jobs/submit_finalize_corpus_v4_latency_v3.sh"
    if (
        executed_batch.is_symlink()
        or not executed_batch.is_file()
        or sha256_file(executed_batch) != finalizer_lock["source_sha256"].get(expected_batch)
    ):
    ...
    output = canonical_output_directory(args.output_root, scheduler["job_id"])
    ...
    try:
        ...
        final = {
            ...
            "provenance": {
                ...
                "scheduler": scheduler,
                "source_git_head": args.expected_finalizer_source_git_head,
        ...
        if validate_finalizer_source(finalizer_lock, args.expected_finalizer_source_git_head) != finalizer_source_sha256:
            raise ValueError("finalizer source changed during analysis")
